      ...
      <div className='grid md:grid-cols-2 md:grid-rows-3 lg:grid-cols-3 lg:grid-rows-2 w-[83vw] h-max pt-4 gap-y-2 lg:ml-32'>
          {
            ...
            articles.map((articles)=>{
              return <>
              <Link key={articles.title} href={`/article/${articles.title}`} className="flex bg-white dark:bg-[#3c3c3c] border border-gray-200 dark:border-black rounded-lg shadow h-20 w-[80vw] md:h-36 md:w-[37vw] lg:h-[20vh] lg:w-[20vw] hover:bg-gray-100 hover:dark:bg-[#313030] overflow-hidden ">
                  <img className="object-cover self-center h-20 w-20 md:w-32 lg:w-[8vw] lg:h-[20vh] rounded-t-lg md:h-36" src={articles.urlToImage ? articles.urlToImage : '/UpFront-placeholder.png'} alt="UpFront" width='20' height='30' />
                  <h5 className="pl-2 mb-2 text-base font-semibold tracking-tight text-gray-900 leading-normal dark:text-gray-300 pt-1"> {articles.title}
                  </h5>
              </Link>
              </>
            })):
